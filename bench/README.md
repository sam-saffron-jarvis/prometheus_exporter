...
The benchmark is procedural so future changes can be measured on the same host rather than compared against a selected historical run.
...
| 3.0 client and Puma server, one finite POST per metric | 12,363 metrics/s | 7,286–12,889 | 81.0 µs |

The finite Puma transport was 13.5% slower by median throughput in the full client/server benchmark while using 13.9% less total CPU per metric. An isolated transport benchmark which reproduced the old client's four socket writes per chunk measured 15,817 metrics/s for WEBrick versus 13,193 metrics/s for finite Puma requests, a 16.6% throughput reduction.

A mixed deployment has a separate and much larger effect. The 3.0 finite client talking to the old WEBrick server measured only 24 requests/s over a persistent connection. WEBrick writes the response headers and two-byte `OK` body separately; Nagle's algorithm and delayed acknowledgements then introduce an approximately 41 ms stall per request. Returning an empty response body removes that stall, but released 2.x servers return `OK`. This is why the README recommends upgrading the exporter server before its producers.
